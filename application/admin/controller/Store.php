...
class Store extends Admin
{
    ...
    public function index()
    {
        ...
        $order = $this->getOrder();
        // 数据列表
        $data_list = StoreModel::hasWhere('sort')->field('name')->where($map)->order($order)->paginate();
        ...
        //分页数组
        $nav = StoreSortModel::where(['status'=>1])->order('sort')->field(['id','name'])->select();
    ...
    public function add()
    {
        ...
        //所属分页
        $list = StoreSortModel::where(['status'=>1])->order('sort')->field(['id','name'])->select();
